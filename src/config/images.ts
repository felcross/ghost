export const servicesImages = [
  "https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=800&q=80",
  "https://images.unsplash.com/photo-1561070791-2526d30994b5?w=800&q=80",
  "https://images.unsplash.com/photo-1471341971476-ae15ff5dd4ea?w=800&q=80",
  "https://images.unsplash.com/photo-1574717024653-61fd2cf4d44d?w=800&q=80",
];

export const testimonialAvatars = [
  "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&q=80",
  "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&q=80",
  "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=100&q=80",
];

// whyWorkWithUsImages removed — WhyWorkWithUs now uses video blocks instead of static images

export const faqImage = "https://images.unsplash.com/photo-1553877522-43269d4ea984?w=600&q=80";

export const videoUrls = {
  hero: "/data/hero.mp4",
  heroPoster: "/mercedes/optimized/hero-poster.jpg",
  showcase: "/data/SP.mp4",
  showcasePoster: "/mercedes/optimized/hero-poster.jpg",
  brazil: "https://videos.pexels.com/video-files/35933207/15244916_2560_1440_60fps.mp4",
  brazilPoster: "https://images.unsplash.com/photo-1483729558449-99ef09a8c325?w=1920&q=80",
  whyWorkWithUsFesta: "/data/festa.mp4",
  whyWorkWithUsClaquete: "/data/claquete.mp4",
};

// Trusted By — only the 8 approved brands
export const trustedByLogos = [
  "MERCEDES-BENZ",
  "NEW BALANCE",
  "FILA",
  "ORIGINAL PENGUIN",
  "RENNER",
  "LIVE!",
  "FERRACINI",
  "ESQUIRE",
];
